...
import myfunnylove.locidnet.lullaby.data.source.PreferenceManager;
import myfunnylove.locidnet.lullaby.event.PlayListNowEvent;
import myfunnylove.locidnet.lullaby.event.PlaySongEvent;
import myfunnylove.locidnet.lullaby.player.IPlayback;
...
import myfunnylove.locidnet.lullaby.ui.widget.ShadowImageView;
import myfunnylove.locidnet.lullaby.utils.AlbumUtils;
...
public class MusicPlayerFragment extends BaseFragment implements MusicPlayerContract.View, IPlayback.Callback {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        new MusicPlayerPresenter(getActivity().getApplicationContext(), AppRepository.getInstance(), this).subscribe();
    }
    ...
    public void onPlayToggleAction(View view) {
        ...
        if (mPlayer.isPlaying()) {
            ...
            PlayList playList =PlayList.fromLocalPlaylist(getString(R.string.mp_local_files_segmented_all), LocalMusicPresenter.getSongs(),LocalMusicPresenter.getSongs().size(),0);
    ...
    public void onSongUpdated(@Nullable Song song) {
        if (song == null) {
            try {
                ...
                buttonPlayToggle.setImageResource(R.drawable.play);
            seekBarProgress.setProgress(0);
        ...
        Drawable drawable = M.getInstance().getApplicationContext().getResources().getDrawable(song.getAlbum());
        Bitmap bitmap  = AlbumUtils.drawableToBitmap(drawable);
        if (bitmap == null) {
...
//            imageViewAlbum.setImageDrawable(bitmap);
        }
       try{
           imageViewAlbum.pauseRotateAnimation();
       }catch (Exception ignored){}
        ...
        if (mPlayer.isPlaying()) {
           try {
            ...
            mHandler.post(mProgressCallback);
            buttonPlayToggle.setImageResource(R.drawable.pause);
        }
    }
    ...
    public void updatePlayToggle(boolean play) {
        buttonPlayToggle.setImageResource(play ? R.drawable.pause : R.drawable.play);
    }
